...
export default function SettingsTab({ userEmail }: { userEmail?: string | null }) {
    ...
    <div className="space-y-4">
      ...
      {!userEmail && (
        <div className="bg-blue-500/5 border border-blue-500/20 rounded-2xl p-5">
          <h3 className="text-white font-semibold mb-2">إضافة بريد إلكتروني 📧</h3>
          <p className="text-slate-400 text-sm mb-4">حسابك مرتبط بـ Pi Network فقط. يمكنك إضافة بريد إلكتروني لتسجيل الدخول به.</p>
          <div className="flex gap-2">
            <input type="email" value={newEmail} onChange={e => setNewEmail(e.target.value)}
            ...
            <button onClick={handleChangeEmail} disabled={isSendingEmail || !newEmail}
              className="px-4 py-2.5 bg-emerald-500 hover:bg-emerald-400 disabled:opacity-50 text-white rounded-xl text-sm font-medium transition-all">
              {isSendingEmail ? '...' : 'إضافة'}
            </button>
          </div>
          {emailSent && (
            <p className="text-emerald-400 text-xs mt-2">✅ تم إرسال رابط التأكيد. تحقق من صندوق الوارد.</p>
          )}
          {emailError && (
