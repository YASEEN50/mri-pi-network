...
interface PageProps { searchParams: Promise<{ type?: string; city?: string; page?: string }> }

export default async function FacilitiesPage({ searchParams }: PageProps) {
  const params = await searchParams
  const t = await getTranslations()
  const locale = await getLocale() as 'ar' | 'en'
  const page = Number(params.page ?? 1), limit = 12
  const where: any = { approvalStatus: ApprovalStatus.APPROVED, deletedAt: null }
  if (params.type) where.type = params.type as FacilityType
    ...
    <div className="min-h-screen bg-slate-950">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <form className="flex flex-wrap gap-3 mb-8" method="GET">
          <input name="city" defaultValue={params.city} placeholder={locale === 'ar' ? 'المدينة...' : 'City...'}
          ...
          <select name="type" defaultValue={params.type ?? ''}
            ...
            {(Object.values(FacilityType) as string[]).map((type: string) => (
